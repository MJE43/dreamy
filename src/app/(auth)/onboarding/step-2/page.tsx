...
import { fetchDilemmas, DilemmaForQuiz /*, DilemmaChoice */ } from "./actions"; // Server Action - Removed unused DilemmaChoice
...
export default function Step2Page() {
  ...
  const handleAnswerChange = (dilemmaCode: string, option: string) => {
    ...
    const choice = dilemma?.details?.choices?.find((c) => c.option === option);
    ...
    if (stageScore) {
      ...
      console.warn(
        `Score not found for dilemma ${dilemmaCode}, option ${option}`
      );
  ...
  const progress =
    totalDilemmas > 0 ? (answeredCount / totalDilemmas) * 100 : 0;
  ...
  const handleNext = () => {
    ...
    try {
      ...
      setError(
        "Could not save progress. Please ensure cookies/session storage are enabled."
      );
    }
  };
  ...
  return (
    <div>
      ...
      <p className="text-muted-foreground mb-4">
        Choose the option that resonates most with you for each dilemma.
      </p>
        ...
        {dilemmas.map((dilemma) => {
          ...
          return (
            <DilemmaCard
              ...
              value={
                selectedScore
                  ? dilemma.details?.choices?.find(
                      (c) => c.score === selectedScore
                    )?.option
                  : undefined
              } // Temporary: Pass A/B based on score for now
